...
const Storage = {
    ...
    saveMatch(match) {
        ...
        if (typeof FirebaseSync !== 'undefined' && FirebaseSync.isConfigured() && FirebaseSync.isAdmin()) {
            FirebaseSync.saveMatchAny(match).catch(err => {
                console.warn('[Storage] saveMatch sync failed', err);
                if (typeof SyncToast !== 'undefined') SyncToast.syncFailed('saveMatch');
            });
        }
    },
    ...
    deleteMatch(id) {
        ...
        // Sync Firebase (non-bloquant)
        if (typeof FirebaseSync !== 'undefined' && FirebaseSync.isConfigured() && FirebaseSync.isAdmin()) {
            FirebaseSync.deleteMatch(id).catch(err => {
                console.warn('[Storage] deleteMatch sync failed', err);
                if (typeof SyncToast !== 'undefined') SyncToast.syncFailed('deleteMatch');
            });
        }
    },
